...
import { Employee, EmployeeEducation, EmployeeAttendance, EmployeeLeave, EmployeeRole } from '../models/employee.model';
...
export class EmployeeService {
  ...
  search(query: string, page: number = 0, size: number = 10, sort?: { sortBy?: string; direction?: 'ASC' | 'DESC'; }): Observable<ApiResponse<PageResponse<Employee>>> {
    const headers = this.getHeaders();
  ...
  searchByRole(role: EmployeeRole, query: string, page: number = 0, size: number = 10): Observable<ApiResponse<PageResponse<Employee>>> {
    const headers = this.getHeaders();
    const params = this.getParams(page, size)
      .set('query', query)
      .set('role', role);
    return this.http.get<ApiResponse<PageResponse<Employee>>>(`${this.API_URL}/search-by-role`, { headers, params });
  }
  ...
  updateEmployeeRole(id: number, role: EmployeeRole): Observable<ApiResponse<Employee>> {
    const headers = this.getHeaders();
    return this.http.put<ApiResponse<Employee>>(`${this.API_URL}/${id}/role`, { role }, { headers });
  }
  ...
  addEducation(employeeId: number, education: EmployeeEducation): Observable<ApiResponse<EmployeeEducation>> {
    const headers = this.getHeaders();
    return this.http.post<ApiResponse<EmployeeEducation>>(`${this.API_URL}/${employeeId}/educations`, education, { headers });
  }
  ...
  updateEducation(employeeId: number, educationId: number, education: EmployeeEducation): Observable<ApiResponse<EmployeeEducation>> {
    const headers = this.getHeaders();
    return this.http.put<ApiResponse<EmployeeEducation>>(`${this.API_URL}/${employeeId}/educations/${educationId}`, education, { headers });
  }

  deleteEducation(employeeId: number, educationId: number): Observable<void> {
    const headers = this.getHeaders();
    return this.http.delete<void>(`${this.API_URL}/${employeeId}/educations/${educationId}`, { headers });
  }

  // Attendance related methods
  addAttendance(employeeId: number, attendance: EmployeeAttendance): Observable<ApiResponse<EmployeeAttendance>> {
    const headers = this.getHeaders();
    return this.http.post<ApiResponse<EmployeeAttendance>>(`${this.API_URL}/${employeeId}/attendances`, attendance, { headers });
  }

  updateAttendance(employeeId: number, attendanceId: number, attendance: EmployeeAttendance): Observable<ApiResponse<EmployeeAttendance>> {
    const headers = this.getHeaders();
    return this.http.put<ApiResponse<EmployeeAttendance>>(`${this.API_URL}/${employeeId}/attendances/${attendanceId}`, attendance, { headers });
  }

  deleteAttendance(employeeId: number, attendanceId: number): Observable<void> {
    const headers = this.getHeaders();
    return this.http.delete<void>(`${this.API_URL}/${employeeId}/attendances/${attendanceId}`, { headers });
  }

  // Leave related methods
  addLeave(employeeId: number, leave: EmployeeLeave): Observable<ApiResponse<EmployeeLeave>> {
    const headers = this.getHeaders();
    return this.http.post<ApiResponse<EmployeeLeave>>(`${this.API_URL}/${employeeId}/leaves`, leave, { headers });
  }

  updateLeave(employeeId: number, leaveId: number, leave: EmployeeLeave): Observable<ApiResponse<EmployeeLeave>> {
    const headers = this.getHeaders();
    return this.http.put<ApiResponse<EmployeeLeave>>(`${this.API_URL}/${employeeId}/leaves/${leaveId}`, leave, { headers });
  }
