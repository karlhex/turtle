  ...
  loadUsers() {
    this.userService.getUsers().subscribe({
      ...
      error: error => {
        ...
        this.snackBar.showError('加载用户列表失败');
      },
    });
  }
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.userService.forcePasswordChange(user.id).subscribe({
          next: () => {
            this.snackBar.showSuccess('操作成功');
            this.loadUsers();
          },
          error: error => {
            console.error('Error forcing password change:', error);
            this.snackBar.showError('操作失败');
          },
        });
      }
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.userService.deleteUser(user.id).subscribe({
          next: () => {
            this.snackBar.showSuccess('用户删除成功');
            this.loadUsers();
          },
          error: error => {
            console.error('Error deleting user:', error);
            this.snackBar.showError('用户删除失败');
          },
        });
      }
    });
